...
public final class SlayerArchitectureRegressionGuard
{
	...
	private static void validatePhysicalVariantRepair()
	{
		...
		final SlayerTravelSelection repaired = coordinator.replacePhysicalItemVariant(
			"dark-beasts",
			21,
			"Eternal slayer ring"
		);
	...
	private static void validateBankTagTravelExclusivity()
	{
		...
		final SlayerTravelSelection selection = SlayerTravelSelection.resolvedItem(
			"smoke-devils",
			1L,
			100,
			"Max cape",
		...
		final SlayerLoadoutItem selected = new SlayerLoadoutItem(
			"Max cape", 100, 1, SlayerLoadoutItem.Status.BANK
		);
		final SlayerLoadoutItem constructionCape = new SlayerLoadoutItem(
			"Construction cape", 101, 1, SlayerLoadoutItem.Status.BANK
		);
		final SlayerLoadoutItem houseTablet = new SlayerLoadoutItem(
			"Teleport to house", 102, 1, SlayerLoadoutItem.Status.BANK
		);
		final SlayerLoadoutItem prayerPotion = new SlayerLoadoutItem(
			"Prayer potion(4)", 103, 1, SlayerLoadoutItem.Status.BANK
		);

		if (!SlayerBankTagLayoutService.shouldSuppressInventoryTravelItem(
			true, 0, 100, prayerPotion, selection
		) || !SlayerBankTagLayoutService.shouldSuppressInventoryTravelItem(
	...
	{
		if (!SlayerRouteCatalog.requiresPreparationBank(
		...
		if (SlayerBankRouteCatalog.getInfernoPreparationBankTarget() == null
			|| !new WorldPoint(2495, 5157, 0).equals(
				SlayerBankRouteCatalog.getInfernoPreparationHotVentDoorTarget()
			)
			|| SlayerTravelRouteCatalog.fallbacksFor(
		 ...
		 */
		if (!SlayerTravelRouteCatalog.requiresExternalTeleportHandoff(
			"Mor Ul Rek east bank", "Ghommal's hilt 6", "Mor Ul Rek"
		) || SlayerTravelRouteCatalog.requiresExternalTeleportHandoff(
		...
		final SlayerLoadoutItem firstCombatItem = new SlayerLoadoutItem(
			"Super restore(4)", 201, 1, SlayerLoadoutItem.Status.BANK
		);
		if (SlayerBankTagLayoutService.shouldSuppressInventoryTravelItem(
		...
		if (!SlayerBankTagLayoutService.usesExternalStagingTravelStrip(-1)
			|| SlayerBankTagLayoutService.usesExternalStagingTravelStrip(0))
		{
			throw new IllegalStateException(
		...
		if (!SlayerBankTagLayoutService.hasDedicatedExtraQuiverAmmoPositionForRegression())
		{
			throw new IllegalStateException(
	...
	{
		final int maxCape = SlayerTravelRouteCatalog.liveCandidatePreference("Max cape");
		final int constructionCape = SlayerTravelRouteCatalog.liveCandidatePreference(
			"Construct. cape"
		);
		final int houseTablet = SlayerTravelRouteCatalog.liveCandidatePreference(
			"Teleport to house"
		);
