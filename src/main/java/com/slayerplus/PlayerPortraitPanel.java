...
public class PlayerPortraitPanel extends JPanel {
  ...
  private static final class TimePalette {
    ...
    private static TimePalette forTime(LocalTime time) {
      ...
      if (minutes < 4 * 60 + 45) {
        return blend(night(), preDawn(), progress(minutes, 3 * 60 + 45, 4 * 60 + 45));
      }
      if (minutes < 6 * 60 + 15) {
        return blend(preDawn(), sunrise(), progress(minutes, 4 * 60 + 45, 6 * 60 + 15));
      }
      if (minutes < 9 * 60) {
        return blend(sunrise(), morning(), progress(minutes, 6 * 60 + 15, 9 * 60));
      }
      if (minutes < 12 * 60 + 30) {
        return blend(morning(), midday(), progress(minutes, 9 * 60, 12 * 60 + 30));
      }
      if (minutes < 16 * 60) {
        return blend(midday(), afternoon(), progress(minutes, 12 * 60 + 30, 16 * 60));
      }
      if (minutes < 18 * 60 + 30) {
        return blend(afternoon(), sunset(), progress(minutes, 16 * 60, 18 * 60 + 30));
      }
      if (minutes < 20 * 60 + 30) {
        return blend(sunset(), dusk(), progress(minutes, 18 * 60 + 30, 20 * 60 + 30));
      }
      return blend(dusk(), night(), progress(minutes, 20 * 60 + 30, 22 * 60));
    }

    private static float progress(int value, int start, int end) {
