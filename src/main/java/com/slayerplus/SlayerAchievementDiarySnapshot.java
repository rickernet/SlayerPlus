...
/** Immutable, client-thread snapshot of claimed Achievement Diary tiers. */
final class SlayerAchievementDiarySnapshot
{
	...
	private SlayerAchievementDiarySnapshot(final Map<String, Integer> completedTiers)
	{
		this.completedTiers = Collections.unmodifiableMap(
			new LinkedHashMap<>(completedTiers)
		);
	}
	...
	static SlayerAchievementDiarySnapshot capture(final Client client)
	{
		if (client == null)
		{
			return EMPTY;
		...
		final Map<String, Integer> tiers = new LinkedHashMap<>();
		put(tiers, "ardougne", client,
		...
		/* Karamja's older tiers are not exposed as completion varbits. */
		tiers.put("karamja",
			client.getVarbitValue(VarbitID.KARAMJA_DIARY_ELITE_COMPLETE) > 0 ? 4 : 0);
		return new SlayerAchievementDiarySnapshot(tiers);
	}

	private static void put(
		final Map<String, Integer> tiers,
		final String region,
		final Client client,
		final int easy,
		final int medium,
	...
	boolean allowsTravelItem(final String itemFamily)
	{
		final String item = normalize(itemFamily);
		/* Lumbridge Elite removes the staff requirement from every fairy ring. */
		if ((item.equals("dramen staff") || item.equals("lunar staff"))
			&& hasTier("lumbridge", 4))
		{
			return false;
		}
		...
		final Requirement requirement = Requirement.forItem(item);
		if (requirement == null)
		{
			return true;
		}
		/*
		 * Every Karamja gloves tier is itself proof that its diary reward was
		 * claimed. In particular, gloves 4 must remain usable when the Elite
		 * completion varbit is unavailable or has not populated after login.
		 * Travel callers still require the physical item to be owned.
		 */
		if (requirement.region.equals("karamja"))
		{
			return true;
		}
		return completedTiers.getOrDefault(requirement.region, 0)
			>= requirement.tier;
	}

	boolean hasTier(final String region, final int tier)
	{
		return completedTiers.getOrDefault(normalize(region), 0)
			>= Math.max(1, Math.min(4, tier));
	}
	...
	/** Slayer Tower bonus in tenths of one percent (25 = 2.5%). */
	...
	boolean allowsWildernessBossLair(final String encounterName)
	{
		final String encounter = normalize(encounterName);
		if (encounter.equals("callisto") || encounter.equals("venenatis")
			|| encounter.equals("vet ion"))
		{
			return hasTier("wilderness", 2);
		}
		if (encounter.equals("artio") || encounter.equals("spindel")
	...
	boolean allowsLoadoutReward(
		final String displayName,
		final Iterable<String> alternatives)
	{
	...
	static SlayerAchievementDiarySnapshot forRegression(
		final Map<String, Integer> completedTiers)
	{
	...
	private static final class Requirement
	{
		...
		private Requirement(final String region, final int tier)
		{
			this.region = region;
			this.tier = tier;
		}

		private static Requirement forItem(final String item)
		{
		...
		private static Requirement numbered(final String region, final String item)
		{
