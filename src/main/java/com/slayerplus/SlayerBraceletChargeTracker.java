...
/**
 * Parses the same authoritative game messages used by RuneLite's Item Charges
 * plugin. Slayer bracelet charges are account-wide, so they cannot be inferred
 * from the bracelet item id itself.
 */
final class SlayerBraceletChargeTracker
{
	...
	private static final Pattern SLAUGHTER_ACTIVATE = Pattern.compile(
	...
	private static final Pattern SLAUGHTER_CHECK = Pattern.compile(
		"Your bracelet of slaughter has (\\d{1,2}) charges? left\\."
	);
	private static final Pattern EXPEDITIOUS_ACTIVATE = Pattern.compile(
		"Your expeditious bracelet helps you progress your slayer (?:task )?faster\\. "
			+ "(?:It has (\\d{1,2}) charges? left\\.|It then crumbles to dust\\.|"
			+ "It then regenerates itself to full charge!)"
	);
	private static final Pattern EXPEDITIOUS_CHECK = Pattern.compile(
	...
	static Update parse(final String message)
	{
		...
		Update update = match(
			message, SLAUGHTER_ACTIVATE, ItemID.BRACELET_OF_SLAUGHTER,
			SLAUGHTER_KEY, true
		);
		if (update != null)
		{
		...
		update = match(
			message, SLAUGHTER_CHECK, ItemID.BRACELET_OF_SLAUGHTER,
			SLAUGHTER_KEY, false
		);
		if (update != null)
		{
		...
		update = match(
			message, EXPEDITIOUS_ACTIVATE, ItemID.EXPEDITIOUS_BRACELET,
			EXPEDITIOUS_KEY, true
		);
		return update != null ? update : match(
			message, EXPEDITIOUS_CHECK, ItemID.EXPEDITIOUS_BRACELET,
			EXPEDITIOUS_KEY, false
		);
	}
	...
	private static Update match(
		...
		final boolean resetWhenNoCount)
	{
