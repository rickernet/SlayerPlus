...
public class SlayerPlusPanel extends PluginPanel{private static final Color BG=new Color(23,23,25);
...
private final JLabel accountNameValue=new JLabel(text(550));
...
private final JButton tagButton=new RoundedButton(text(551),8);
private final JButton sessionButton=new RoundedButton("Start Slayer session",8);
private final JButton discordButton=new RoundedButton(text(552),8);
...
private final JPanel positionPanel=new RoundedPanel(SURFACE,SURFACE_BORDER,8);
private final JLabel positioningValue=muted("");
private final JPanel travelPanel=new RoundedPanel(new Color(49,39,24),new Color(126,88,40),8);
private final JLabel bankTravel=detailValue("");
private final JLabel bankNote=muted("");
...
public SlayerPlusPanel(){this(null,null,null,null,null);
}public SlayerPlusPanel(Runnable bankTagAction,Consumer<TaskVariant>bankVariantAction){this(bankTagAction,bankVariantAction,null,null,null);
}public SlayerPlusPanel(Runnable bankTagAction,Consumer<TaskVariant>bankVariantAction,Runnable sessionToggleAction){this(bankTagAction,bankVariantAction,sessionToggleAction,null,null);
}public SlayerPlusPanel(Runnable bankTagAction,Consumer<TaskVariant>bankVariantAction,Runnable sessionToggleAction,SlayerPlusConfig config,ConfigManager configs){this.bankTagAction=bankTagAction;
...
setBackground(BG);
setBorder(BorderFactory.createEmptyBorder(OUTER_PANEL_PADDING,OUTER_PANEL_PADDING,OUTER_PANEL_PADDING,OUTER_PANEL_PADDING));
addComponentListener(new ComponentAdapter(){@Override public void componentResized(ComponentEvent event){scheduleResponsiveRewrap();
}});
...
showView(BANK_VIEW);
}private JButton createDiscordButton(){discordButton.setFont(new Font("SansSerif",Font.BOLD,12));
...
discordButton.setToolTipText(text(553));
discordButton.addActionListener(event->LinkBrowser.browse(DISCORD_INVITE_URL));
return discordButton;
}private JPanel createHeader(){JPanel panel=new JPanel(new BorderLayout());
...
return panel;
}private JPanel createPlayerCard(){JPanel card=card();
...
return card;
}private JPanel sideStatRow(String labelText,JLabel valueLabel){JPanel row=new JPanel();
...
row.add(valueLabel);
return row;
}private JPanel separator(){JPanel line=new JPanel();
...
return line;
}private JPanel createTaskCard(){JPanel card=card();
card.setLayout(new BoxLayout(card,BoxLayout.Y_AXIS));
...
return card;
}private JPanel createViewTabs(){JPanel buttons=new JPanel(new GridLayout(1,2,6,0));
...
return buttons;
}private void configureViewButton(JButton button){button.setFont(new Font("SansSerif",Font.BOLD,11));
...
}private void showView(String view){boolean settings=SETTINGS_VIEW.equals(view);
...
if(settings){refreshSettingsControls();
}refreshLayout();
}private JPanel createSettingsCard(){JPanel settings=card();
...
setWrappedText(description,text(554));
...
playstyleSetting=new JComboBox<>(new Preference.Playstyle[]{Preference.Playstyle.FAST_XP,Preference.Playstyle.PROFIT});
...
JPanel modeSection=settingsSection("SLAYER MODE",text(555));
modeSection.add(fullWidth(settingRow("Mode",workflowSetting)));
bonusMasterRow=settingRow(text(556),bonusMasterSetting);
pointBoostExplanation=muted("");
setWrappedText(pointBoostExplanation,text(557));
...
JPanel loadoutSection=settingsSection(text(558),text(559));
...
workflowSetting.addActionListener(event->{saveSetting("slayerWorkflow",workflowSetting.getSelectedItem());
refreshPointBoostSettingEnabled();
});
bonusMasterSetting.addActionListener(event->saveSetting(text(560),bonusMasterSetting.getSelectedItem()));
combatStyleSetting.addActionListener(event->saveSetting("combatStylePreference",combatStyleSetting.getSelectedItem()));
helmSetting.addActionListener(event->saveSetting(HelmetPreference.CONFIG_KEY,helmSetting.getSelectedItem()));
...
return settings;
}private void refreshPointBoostSettingEnabled(){if(workflowSetting==null||bonusMasterSetting==null){return;
}boolean pointBoosting=workflowSetting.getSelectedItem()==Preference.Workflow.TURAEL_POINT_BOOST;
bonusMasterSetting.setEnabled(pointBoosting);
if(pointBoostControls!=null){pointBoostControls.setVisible(pointBoosting);
if(pointBoostControls.getParent()!=null){pointBoostControls.getParent().revalidate();
pointBoostControls.getParent().repaint();
}refreshLayout();
}}private JPanel settingRow(String labelText,JComboBox<?>selector){JPanel row=new JPanel(new BorderLayout(6,4));
row.setOpaque(false);
...
selector.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(SURFACE_BORDER),BorderFactory.createEmptyBorder(1,4,1,4)));
row.add(label,BorderLayout.NORTH);
row.add(selector,BorderLayout.CENTER);
return row;
}private JPanel settingsSection(String title,String description){JPanel section=new RoundedPanel(SURFACE,SURFACE_BORDER,8);
section.setLayout(new BoxLayout(section,BoxLayout.Y_AXIS));
section.setBorder(BorderFactory.createEmptyBorder(SECTION_PADDING,SECTION_PADDING,SECTION_PADDING,SECTION_PADDING));
section.add(fullWidth(heading(title)));
if(description!=null&&!description.trim().isEmpty()){section.add(Box.createVerticalStrut(SECTION_GAP));
JLabel help=muted("");
setWrappedText(help,description);
section.add(fullWidth(help));
section.add(Box.createVerticalStrut(CARD_GAP));
}return section;
}private void saveSetting(String key,Object value){if(updatingSettings||configs==null||value==null){return;
}configs.setConfiguration(SlayerPlusConfig.GROUP,key,value);
}private void refreshSettingsControls(){if(config==null||combatStyleSetting==null){return;
}updatingSettings=true;
try{workflowSetting.setSelectedItem(config.slayerWorkflow());
...
}}public void configureOwnedSlayerHelmets(List<String>helmetNames){ownedHelms.clear();
if(helmetNames!=null){ownedHelms.addAll(helmetNames);
}if(helmSetting!=null){updatingSettings=true;
try{rebuildSlayerHelmetSetting();
}finally{updatingSettings=false;
}}}private void rebuildSlayerHelmetSetting(){if(helmSetting==null){return;
}String configured=configs==null?HelmetPreference.COMBAT_ACHIEVEMENT:HelmetPreference.normalize(configs.getConfiguration(SlayerPlusConfig.GROUP,HelmetPreference.CONFIG_KEY));
...
for(String helmetName:ownedHelms){helmSetting.addItem(helmetName);
}helmSetting.setSelectedItem(ownedHelms.contains(configured)||HelmetPreference.isRandom(configured)?configured:HelmetPreference.COMBAT_ACHIEVEMENT);
}private JPanel createBankTagCard(){JPanel bankCard=card();
...
return bankCard;
}private JPanel createBankPreparationPanel(){return configurePreparationPanel(preparationPanel,preparationValue,bankPreparationNote,"Spell setup");
}private JPanel createBankPositioningPanel(){positionPanel.setLayout(new BoxLayout(positionPanel,BoxLayout.Y_AXIS));
positionPanel.setBackground(SURFACE);
applySectionPadding(positionPanel);
positionPanel.setAlignmentX(LEFT_ALIGNMENT);
JLabel heading=heading("TASK METHOD");
positioningValue.setAlignmentX(LEFT_ALIGNMENT);
positionPanel.add(fullWidth(heading));
positionPanel.add(Box.createVerticalStrut(SECTION_GAP));
positionPanel.add(fullWidth(positioningValue));
positionPanel.setVisible(false);
return positionPanel;
}private JPanel configurePreparationPanel(JPanel panel,JLabel valueLabel,JLabel noteLabel,String headingText){valueLabel.setFont(new Font("SansSerif",Font.BOLD,BODY_TEXT_SIZE));
...
return panel;
}private JPanel createBankTravelRecommendationPanel(){travelPanel.setLayout(new BoxLayout(travelPanel,BoxLayout.Y_AXIS));
travelPanel.setBackground(SURFACE);
applySectionPadding(travelPanel);
travelPanel.setAlignmentX(LEFT_ALIGNMENT);
JLabel heading=heading("NEXT STEP");
bankTravel.setAlignmentX(LEFT_ALIGNMENT);
bankNote.setAlignmentX(LEFT_ALIGNMENT);
bankTravel.setForeground(ACCENT);
bankTravel.setFont(new Font("SansSerif",Font.BOLD,12));
bankNote.setForeground(TEXT);
setOptionalText(bankTravel,"");
setOptionalText(bankNote,"");
travelPanel.add(fullWidth(heading));
travelPanel.add(Box.createVerticalStrut(SECTION_GAP));
travelPanel.add(fullWidth(bankTravel));
travelPanel.add(Box.createVerticalStrut(SECTION_GAP));
travelPanel.add(fullWidth(bankNote));
travelPanel.setVisible(false);
return travelPanel;
}private JPanel createBankDartRecommendationPanel(){dartPanel.setLayout(new BoxLayout(dartPanel,BoxLayout.Y_AXIS));
...
return dartPanel;
}private JPanel createTaskVariantSelector(){taskVariantPanel.setLayout(new BorderLayout(0,4));
...
taskVariantSelector.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(SURFACE_BORDER),BorderFactory.createEmptyBorder(2,5,2,5)));
taskVariantSelector.setPreferredSize(new Dimension(1,29));
taskVariantSelector.addActionListener(event->{if(updatingTaskVariant||bankVariantAction==null){return;
}int index=taskVariantSelector.getSelectedIndex();
if(index<0||index>=visibleTaskVariants.size()){return;
}bankTagReady=false;
tagButton.setText("Refreshing setup…");
tagButton.setEnabled(false);
tagButton.setToolTipText(text(561));
...
}public void configureTaskVariants(String assignment,TaskVariant selectedVariant){List<TaskVariant>variants=VariantCatalog.getAvailableVariants(assignment);
updatingTaskVariant=true;
try{visibleTaskVariants.clear();
taskVariantSelector.removeAllItems();
for(TaskVariant variant:variants){visibleTaskVariants.add(variant);
...
}finally{updatingTaskVariant=false;
}refreshLayout();
}private JPanel createBankTagActionButton(){JPanel panel=new JPanel();
panel.setLayout(new BoxLayout(panel,BoxLayout.Y_AXIS));
panel.setBackground(BG);
...
tagButton.setEnabled(false);
tagButton.addActionListener(event->{if(bankTagAction==null||!bankTagReady){return;
...
sessionButton.addActionListener(event->{if(sessionToggleAction==null||(!active&&!sessionAvailable)){return;
}sessionButton.setText(active?"Stopping…":"Starting…");
sessionButton.setEnabled(false);
...
return panel;
}private void updateDartRecommendation(Recommendation recommendation,KitPlan loadout){boolean profitBlowpipe=recommendation!=null&&recommendation.getStrategy()!=null&&recommendation.getStrategy().getCostPolicy()==TaskStrategy.CostPolicy.EFFICIENT&&loadout!=null&&containsLoadoutItem(loadout.getEquipmentItems(),"blowpipe");
if(!profitBlowpipe){dartPanel.setVisible(false);
setOptionalText(dartValue,"");
setOptionalText(dartNote,"");
return;
...
if(!ownedDartName.isEmpty()&&!ownedDartName.toLowerCase().contains("dragon dart")){recommendedDarts=pluralizeDartName(ownedDartName);
recommendedStackOwned=true;
}else{recommendedDarts=text(562);
...
}private void setBankLayoutHeading(String layoutTitle){String safeTitle=layoutTitle==null?"":layoutTitle.trim();
int separatorIndex=safeTitle.indexOf(" • ");
if(separatorIndex>0){setWrappedText(bankLayoutTitle,SlayerDisplayText.bankSetupTitle(safeTitle.substring(0,separatorIndex).trim()));
setOptionalText(bankLayoutSubtitle,safeTitle.substring(separatorIndex+3).trim());
}else{setWrappedText(bankLayoutTitle,SlayerDisplayText.bankSetupTitle(safeTitle));
setOptionalText(bankLayoutSubtitle,"");
}}private void updateBankTagCard(KitPlan plan){KitPlan safePlan=plan==null?KitPlan.empty():plan;
setBankLayoutHeading(safePlan.getLayoutTitle());
bankTagReady=safePlan.hasConcreteItems()&&bankTagAction!=null;
if(bankTagReady){tagButton.setToolTipText(text(563));
tagButton.setText(text(551));
tagButton.setBackground(BLUE);
tagButton.setEnabled(true);
}else{String unavailableStatus=safePlan.hasVisualLayout()?text(564):compactBankTagStatus(safePlan.getOwnedStatus());
tagButton.setToolTipText(unavailableStatus);
tagButton.setText("Bank tag unavailable");
tagButton.setBackground(DISABLED_BG);
tagButton.setEnabled(false);
}}private JPanel card(){JPanel panel=new RoundedPanel(PANEL,LINE,10);
panel.setBorder(BorderFactory.createEmptyBorder(CARD_PADDING,CARD_PADDING,CARD_PADDING,CARD_PADDING));
panel.setAlignmentX(LEFT_ALIGNMENT);
panel.setMaximumSize(new Dimension(Integer.MAX_VALUE,Integer.MAX_VALUE));
return panel;
}private static void applySectionPadding(JPanel panel){panel.setBorder(BorderFactory.createEmptyBorder(SECTION_PADDING,SECTION_PADDING,SECTION_PADDING,SECTION_PADDING));
}private JPanel standardRow(String name,JLabel value){JPanel panel=new JPanel(new BorderLayout(8,0));
...
return panel;
}private static<T extends java.awt.Component>T fullWidth(T component){if(component instanceof javax.swing.JComponent){javax.swing.JComponent swingComponent=(javax.swing.JComponent)component;
...
}private static JLabel heading(String text){JLabel label=new JLabel(normalizeHeading(text));
label.setForeground(ACCENT);
label.setFont(new Font("SansSerif",Font.BOLD,CARD_HEADING_SIZE));
return label;
}private static JLabel sectionHeading(String text){JLabel label=new JLabel(normalizeHeading(text));
label.setForeground(MUTED);
label.setFont(new Font("SansSerif",Font.BOLD,SECTION_HEADING_SIZE));
return label;
}private static String normalizeHeading(String text){return text==null?"":text.trim().toUpperCase(Locale.ENGLISH);
}private static JLabel value(String text,int size){JLabel label=new JLabel(text);
label.setForeground(TEXT);
label.setFont(new Font("SansSerif",Font.BOLD,size));
return label;
}private static JLabel detailValue(String text){JLabel label=new JLabel(text);
label.setForeground(TEXT);
label.setFont(new Font("SansSerif",Font.PLAIN,BODY_TEXT_SIZE));
return label;
}private static JLabel muted(String text){JLabel label=new JLabel(text);
label.setForeground(MUTED);
label.setFont(new Font("SansSerif",Font.PLAIN,BODY_TEXT_SIZE));
return label;
}private static void setWrappedText(JLabel label,String text){setWrappedText(label,text,0);
}private static void setWrappedText(JLabel label,String text,int wrapWidthCap){String safeText=text==null||text.trim().isEmpty()?"—":text;
...
}private static int responsiveWrapWidth(JLabel label,int requestedCap){int availableWidth=0;
Container parent=label.getParent();
if(parent!=null&&parent.getWidth()>0){Insets parentInsets=parent.getInsets();
availableWidth=parent.getWidth()-parentInsets.left-parentInsets.right;
}if(availableWidth<=0&&label.getWidth()>0){availableWidth=label.getWidth();
}if(availableWidth<=0){availableWidth=INITIAL_WRAP_WIDTH;
}Insets labelInsets=label.getInsets();
availableWidth-=labelInsets.left+labelInsets.right;
if(requestedCap>0){availableWidth=Math.min(availableWidth,requestedCap);
}return Math.max(MIN_RESPONSIVE_WRAP_WIDTH,availableWidth);
}private static String buildWrappedHtml(JLabel label,String text,int wrapWidth){java.awt.FontMetrics metrics=label.getFontMetrics(label.getFont());
...
index<lines.size();
index++){if(index>0){html.append("<br>");
}html.append(escapeHtml(lines.get(index)));
}if(bold){html.append("</b>");
}return html.append("</html>").toString();
}private static List<String>wrapText(java.awt.FontMetrics metrics,String text,int requestedWidth){int width=Math.max(1,requestedWidth);
List<String>lines=new ArrayList<>();
StringBuilder line=new StringBuilder();
String[]words=text.trim().split("\\s+");
for(String word:words){String candidate=line.length()==0?word:line+" "+word;
if(metrics.stringWidth(candidate)<=width){line.setLength(0);
line.append(candidate);
continue;
}if(line.length()>0){lines.add(line.toString());
line.setLength(0);
}if(metrics.stringWidth(word)<=width){line.append(word);
continue;
}StringBuilder fragment=new StringBuilder();
for(int index=0;
index<word.length();
index++){char character=word.charAt(index);
if(fragment.length()>0&&metrics.stringWidth(fragment.toString()+character)>width){lines.add(fragment.toString());
...
}private static void updateDynamicLabelHeight(JLabel label){Dimension preferred=label.getPreferredSize();
...
label.revalidate();
}private static String escapeHtml(String value){return value.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace("\"","&quot;").replace("'","&#39;");
}private static String pluralizeDartName(String dartName){if(dartName==null){return "darts";
}String trimmed=dartName.trim();
return trimmed.toLowerCase().endsWith(" dart")?trimmed+"s":trimmed;
}private static boolean containsLoadoutItem(List<KitItem>items,String nameFragment){return!findLoadoutItemName(items,nameFragment).isEmpty();
}private static String findLoadoutItemName(List<KitItem>items,String nameFragment){if(items==null||nameFragment==null){return "";
}String fragment=nameFragment.trim().toLowerCase();
for(KitItem item:items){if(item==null||item.getDisplayName()==null){continue;
}String displayName=item.getDisplayName().trim();
if(!displayName.isEmpty()&&displayName.toLowerCase().contains(fragment)){return displayName;
}}return "";
}private static String cleanDetail(String value){if(value==null){return "";
}String text=value.trim();
return text.equals("—")?"":text;
}private static String compactPreparationHeadline(String value){return cleanDetail(value).replace(text(1079),text(1080)).replace(text(1081),"Setup needed •").trim();
}private static String compactPreparationDetail(String value){return cleanDetail(value).replace("Load ","Add ").replace(text(95),"").replace("rune pouch","pouch").trim();
}private static String displayTaskMethod(Recommendation recommendation){if(recommendation==null){return "";
}String recommended=cleanDetail(recommendation.getMethod());
if(!recommended.isEmpty()){return recommended;
}if(recommendation.getStrategy()!=null){String authored=cleanDetail(recommendation.getStrategy().getMethod());
if(!authored.isEmpty()){return authored;
}}return text(568)+" guidance.";
}static String compactSessionStatus(boolean active,boolean available,String value){String text=cleanDetail(value);
if(text.isEmpty()){return "";
}String normalized=text.toLowerCase();
if(active&&normalized.contains("ghommal")&&normalized.contains("mor ul rek")){return "";
}if(active&&normalized.contains("hot vent door")&&normalized.contains("pass")){return text(569)+text(570);
}if(active&&(normalized.startsWith(text(571))||normalized.startsWith("session active")||normalized.startsWith("routing to")||normalized.contains("follow the route")||normalized.contains(text(572))||normalized.contains("route active"))){return "";
}if(!active&&available&&normalized.contains(text(573))){return "";
}return text.replace(text(574),text(575)).replace(text(576),text(577)).trim();
}private static String playerFacingBankTagStatus(String value){String text=compactBankTagStatus(value);
if(text.isEmpty()){return "";
}String normalized=text.toLowerCase(java.util.Locale.ENGLISH);
if(normalized.contains(text(578))||normalized.contains("arranged slots")||normalized.contains("unresolved recommendation")||normalized.contains("created")||normalized.contains("updated")||normalized.contains("replaced")){return "";
}if(normalized.contains("open your bank")){return text(579);
}if(normalized.contains("unavailable")){return text(580);
}if(normalized.contains("teleport omitted")||normalized.contains("no teleport")){return text(581);
}return "";
}private static String compactBankTagStatus(String value){return cleanDetail(value).replace(text(582)+" tag.",text(583)).replace(text(584),text(564)).trim();
}private static void setOptionalText(JLabel label,String text){String safeText=cleanDetail(text);
boolean visible=!safeText.isEmpty();
label.setVisible(visible);
if(visible){setWrappedText(label,safeText);
}else{label.setText("");
...
label.revalidate();
}}private void scheduleResponsiveRewrap(){if(responsiveRewrapScheduled){return;
}responsiveRewrapScheduled=true;
SwingUtilities.invokeLater(()->{responsiveRewrapScheduled=false;
if(rewrapDynamicLabels(this)){refreshLayout();
}});
}private static boolean rewrapDynamicLabels(Component component){boolean changed=false;
if(component instanceof JLabel){JLabel label=(JLabel)component;
Object rawText=label.getClientProperty(WRAP_TEXT_PROPERTY);
if(rawText instanceof String){Object capValue=label.getClientProperty(WRAP_WIDTH_CAP_PROPERTY);
...
if(!(renderedWidth instanceof Integer)||(Integer)renderedWidth!=wrapWidth){label.putClientProperty(WRAP_WIDTH_PROPERTY,wrapWidth);
label.setText(buildWrappedHtml(label,(String)rawText,wrapWidth));
updateDynamicLabelHeight(label);
changed=true;
}}}if(component instanceof Container){for(Component child:((Container)component).getComponents()){changed|=rewrapDynamicLabels(child);
}}return changed;
}private void refreshLayout(){refreshPanelHeight(taskCard);
refreshPanelHeight(assignedAreaRow);
refreshPanelHeight(taskVariantPanel);
refreshPanelHeight(preparationPanel);
refreshPanelHeight(positionPanel);
refreshPanelHeight(travelPanel);
refreshPanelHeight(dartPanel);
updateActiveViewSize();
revalidate();
repaint();
if(isDisplayable()){scheduleResponsiveRewrap();
}}private static void refreshPanelHeight(JPanel panel){if(panel==null){return;
}panel.invalidate();
Dimension preferred=panel.getPreferredSize();
panel.setMaximumSize(new Dimension(Integer.MAX_VALUE,preferred.height));
panel.revalidate();
}void refreshResponsiveWrappingForTest(){if(rewrapDynamicLabels(this)){refreshLayout();
}}private void updateActiveViewSize(){if(bankTagCard==null||settingsCard==null){return;
...
}public void showAccountName(String accountName){String safeName=accountName==null||accountName.trim().isEmpty()?"ACCOUNT LOADING":accountName.trim();
accountNameValue.setText(safeName);
accountNameValue.setToolTipText(safeName);
refreshLayout();
}public void setPortrait(BufferedImage portrait){portraitPanel.setPortrait(portrait);
}public void disposeResources(){portraitPanel.disposeTimers();
}public void showTask(String name,int remaining,int initialAmount,String master,String assignedLocation,int points,int streak){showTask(name,remaining,initialAmount,master,assignedLocation,points,streak,"Konar".equalsIgnoreCase(master));
}public void showTask(String name,int remaining,int initialAmount,String master,String assignedLocation,int points,int streak,boolean showAssignedArea){String displayTaskName=SlayerDisplayText.assignment(name);
setWrappedText(assignment,displayTaskName);
taskProgress.setText(initialAmount>0?remaining+" / "+initialAmount+" remaining":remaining+" remaining");
masterValue.setText(master);
pointsValue.setText(Integer.toString(points));
streakValue.setText(Integer.toString(streak));
setWrappedText(assignedAreaValue,showAssignedArea&&assignedLocation!=null?assignedLocation:"—",COMPACT_VALUE_WRAP_WIDTH);
assignedAreaRow.setVisible(showAssignedArea);
positionPanel.setVisible(false);
setOptionalText(positioningValue,"");
refreshLayout();
}public void showRecommendation(Recommendation recommendation){showRecommendation(recommendation,new KitPlan("—","—","No item scan available"),true);
}public void showRecommendation(Recommendation recommendation,KitPlan loadout,boolean routingEnabled){if(recommendation==null){updateBankTagCard(loadout);
updateDartRecommendation(null,loadout);
positionPanel.setVisible(false);
setOptionalText(positioningValue,"");
refreshLayout();
return;
}String method=displayTaskMethod(recommendation);
setOptionalText(positioningValue,method);
positionPanel.setVisible(!method.trim().isEmpty());
...
refreshLayout();
}public void showPositioningNote(String note){}public void showPreparation(PreparationCatalog.PreparationPlan plan){PreparationCatalog.PreparationPlan safePlan=plan==null?PreparationCatalog.PreparationPlan.none():plan;
if(!safePlan.isActive()){preparationPanel.setVisible(false);
...
String actionableDetail=safePlan.isReady()?"":compactPreparationDetail(safePlan.getDetail());
...
refreshLayout();
}public void showTravelRecommendation(String itemName,String note){String safeName=itemName==null?"":itemName.trim();
String safeNote=cleanDetail(note);
if(safeName.isEmpty()&&safeNote.isEmpty()){travelPanel.setVisible(false);
setOptionalText(bankTravel,"");
setOptionalText(bankNote,"");
refreshLayout();
return;
}if(safeName.isEmpty()){setOptionalText(bankTravel,safeNote);
setOptionalText(bankNote,"");
travelPanel.setVisible(true);
refreshLayout();
return;
}setOptionalText(bankTravel,playerFacingTravelAction(safeName));
String authoredNote=playerFacingTravelNote(safeName);
String nameKey=safeName.toLowerCase(java.util.Locale.ENGLISH);
boolean instructionSuppliesItsOwnNote=nameKey.startsWith("follow ")||nameKey.startsWith("walk ")||nameKey.startsWith("enter ")||nameKey.startsWith("climb ")||nameKey.startsWith("open ");
setOptionalText(bankNote,authoredNote.isEmpty()&&instructionSuppliesItsOwnNote?safeNote:authoredNote);
travelPanel.setVisible(true);
refreshLayout();
}static String playerFacingTravelAction(String value){String text=cleanDetail(value);
String normalized=text.toLowerCase(java.util.Locale.ENGLISH);
if(normalized.contains("light creature")&&normalized.contains("chasm")){return text(585);
}if(normalized.startsWith("climb ")||normalized.startsWith("enter ")||normalized.startsWith("follow ")||normalized.startsWith("walk ")||normalized.startsWith("open ")){return text;
}if(normalized.contains("ghommal")){return "Withdraw and use "+text;
}return text.isEmpty()?"":"Use "+text;
}static String playerFacingTravelNote(String value){String text=cleanDetail(value);
String normalized=text.toLowerCase(java.util.Locale.ENGLISH);
if(normalized.contains("ghommal")){return text(586);
}if(normalized.contains("sapphire lantern")){return text(587)+" skull.";
}if(normalized.contains("light creature")&&normalized.contains("chasm")){return text(588);
}return "";
}public void showSessionState(boolean active,boolean available,String status){active=active;
...
refreshLayout();
}public void showBankTagStatus(String status){String visibleStatus=playerFacingBankTagStatus(status);
...
refreshLayout();
}public void showRouteStatus(String status){travelPanel.setToolTipText(cleanDetail(status));
}public void showPointBoostStatus(String status){setOptionalText(pointBoostStatus,status);
refreshLayout();
}public void showNoTask(int points,int streak,String master){setWrappedText(assignment,"No active task");
...
refreshLayout();
}public void showLoggedOut(){showAccountName(text(550));
setPortrait(null);
...
showPositioningNote("");
showPreparation(PreparationCatalog.PreparationPlan.none());
showSessionState(false,false,text(576));
refreshLayout();
}private static final class RoundedPanel extends JPanel{private final Color borderColor;
private final int arc;
private RoundedPanel(Color fillColor,Color borderColor,int arc){this.borderColor=borderColor;
this.arc=arc;
setBackground(fillColor);
setOpaque(false);
}@Override protected void paintComponent(Graphics graphics){Graphics2D graphics2D=(Graphics2D)graphics.create();
try{graphics2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
...
}finally{graphics2D.dispose();
}super.paintComponent(graphics);
}}private static final class RoundedButton extends JButton{private final int arc;
private RoundedButton(String text,int arc){super(text);
this.arc=arc;
setOpaque(false);
...
}else if(getModel().isRollover()&&isEnabled()){fill=fill.brighter();
}graphics2D.setColor(fill);
graphics2D.fillRoundRect(0,0,getWidth(),getHeight(),arc,arc);
}finally{graphics2D.dispose();
}super.paintComponent(graphics);
}}int currentTaskCardMaximumHeightForTest(){return taskCard.getMaximumSize().height;
}int currentTaskNamePreferredHeightForTest(){return assignment.getPreferredSize().height;
}int bankTravelMaximumHeightForTest(){return travelPanel.getMaximumSize().height;
}void setCurrentTaskContainerWidthForTest(int width){Container parent=assignment.getParent();
if(parent!=null){parent.setSize(width,Math.max(1,parent.getHeight()));
refreshResponsiveWrappingForTest();
}}int currentTaskRenderedWrapWidthForTest(){Object width=assignment.getClientProperty(WRAP_WIDTH_PROPERTY);
