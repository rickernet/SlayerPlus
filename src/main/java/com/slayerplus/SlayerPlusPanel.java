...
}public SlayerPlusPanel(Runnable bankTagAction,Consumer<TaskVariant>bankVariantAction,Runnable travelHighlightToggleAction,SlayerPlusConfig config,ConfigManager configs){this.bankTagAction=bankTagAction;
...
settings.add(Box.createVerticalStrut(SECTION_PADDING));
JPanel devSection=settingsSection("DEVELOPER PREVIEW","Simulate any task or boss to test its loadout and routing without being assigned it.");
...
devPreviewSetting.addActionListener(event->{if(updatingDevPreview||devPreviewAction==null){return;
...
}}public void setDevPreviewOptions(List<String>taskNames,Consumer<String>action){devPreviewAction=action;
updatingDevPreview=true;
try{devPreviewSetting.removeAllItems();
devPreviewSetting.addItem("");
if(taskNames!=null){for(String name:taskNames){devPreviewSetting.addItem(name);
}}}finally{updatingDevPreview=false;
}}public void configureOwnedSlayerHelmets(List<String>helmetNames){if(!ownedHelms.equals(helmetNames==null?java.util.Collections.emptyList():helmetNames)){ownedHelms.clear();
...
for(int i=0;sameOptions&&i<ownedHelms.size();i++){sameOptions=ownedHelms.get(i).equals(helmSetting.getItemAt(i+2));
}if(!sameOptions){helmSetting.removeAllItems();
...
}}Object selected=ownedHelms.contains(configured)||HelmetPreference.isRandom(configured)?configured:HelmetPreference.COMBAT_ACHIEVEMENT;
if(!java.util.Objects.equals(selected,helmSetting.getSelectedItem())){helmSetting.setSelectedItem(selected);
}
...
boolean sameOptions=visibleTaskVariants.equals(variants)&&taskVariantSelector.getItemCount()==variants.size();
for(int i=0;sameOptions&&i<variants.size();i++){sameOptions=VariantCatalog.getOptionLabel(assignment,variants.get(i)).equals(taskVariantSelector.getItemAt(i));
}int selectedIndex=Math.max(0,variants.indexOf(selectedVariant));
if(sameOptions&&taskVariantSelector.getSelectedIndex()==selectedIndex){return;
}
updatingTaskVariant=true;
try{if(!sameOptions){visibleTaskVariants.clear();
...
highlightButton.setToolTipText("Highlight the teleport item and destination for your current task.");
highlightButton.addActionListener(event->{if(travelHighlightToggleAction==null){return;
}travelHighlightToggleAction.run();
});
panel.add(fullWidth(highlightButton));
return panel;
}public void showTravelHighlightState(boolean active){highlightActive=active;
...
}void runBatchedUpdate(Runnable update){layoutUpdateDepth++;
try{update.run();
}finally{layoutUpdateDepth--;
if(layoutUpdateDepth==0&&layoutRefreshPending){layoutRefreshPending=false;
refreshLayout();
}}
}private void refreshLayout(){if(layoutUpdateDepth>0){layoutRefreshPending=true;
return;
...
refreshLayout();
}public void showRecommendation(Recommendation recommendation,KitPlan loadout){if(recommendation==null){updateBankTagCard(loadout);
...
refreshLayout();
}public void showPreparation(PreparationCatalog.PreparationPlan plan){PreparationCatalog.PreparationPlan safePlan=plan==null?PreparationCatalog.PreparationPlan.none():plan;
...
streakValue.setText("—");
clearActiveTaskDisplay();
}private void clearActiveTaskDisplay(){setWrappedText(assignedAreaValue,"—",COMPACT_VALUE_WRAP_WIDTH);
assignedAreaRow.setVisible(false);
