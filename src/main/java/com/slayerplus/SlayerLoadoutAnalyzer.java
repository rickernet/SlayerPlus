...
}public void invalidateBankSnapshot(){cachedBankSnapshotSource=Collections.emptyMap();
bankOwnedItems=Collections.emptyList();
...
addStrategyCandidate(candidates,effectiveStrategy,pool,effectiveTask,config.playstyle(),true);
...
addStrategyCandidate(candidates,candidate,pool,effectiveTask,config.playstyle(),false);
...
return new Recommendation(effectiveLocation,resolvedMethod,ownedReason+best.strategy.getRationale(),effectiveTarget.getTravel(),effectiveTarget.getCannon(),recommendation.getRequirements(),effectiveTarget.getRestriction(),recommendation.getDestination(),selectedStrategy);
}private static void addStrategyCandidate(List<StrategyCandidate>candidates,TaskStrategy strategy,List<OwnedItem>pool,String encounter,Preference.Playstyle playstyle,boolean originalAutomatic){if(strategy==null||!strategy.isReviewed()){return;
...
return composition==null?!requireDefinition:composition.getPlaceholderTemplateId()==-1;
}catch(RuntimeException ignored){return!requireDefinition;
}}private Set<String>getMatchNames(int itemId,String displayName){Set<String>names=new LinkedHashSet<>();
...
}}}catch(RuntimeException ignored){}return Collections.unmodifiableSet(names);
...
}}private String getItemName(int itemId){try{ItemComposition composition=itemManager.getItemComposition(itemId);
...
}}private ItemEquipmentStats getEquipmentStats(int itemId){try{ItemStats stats=itemManager.getItemStats(itemId);
...
}private static OwnedItem selectRecommendedWeapon(String assignment,TaskStrategy strategy,CombatStyle style,List<Requirement>requirements,List<OwnedItem>equipped,List<OwnedItem>pool){Requirement weaponRequirement=requirementForSlot(requirements,EquipmentInventorySlot.WEAPON);
...
}private static List<KitItem>buildEquipmentLayout(String task,TaskStrategy strategy,CombatStyle style,List<Requirement>requirements,List<OwnedItem>equipped,List<OwnedItem>pool,boolean scanned,OwnedItem weapon){String normalizedTask=normalize(task);
...
layout.add(auditEquipment(task,EquipmentInventorySlot.AMULET,requirementForSlot(requirements,EquipmentInventorySlot.AMULET),style==CombatStyle.MAGIC?"Magic amulet":style==CombatStyle.RANGED?text(319):"Strength amulet",strategy,style,equipped,pool,scanned,false,weapon));
layout.add(normalizedTask.equals("araxxor")?buildAraxxorSwitchAmmoItem(style,weapon,strategy,pool,scanned):normalizedTask.equals(text(2))?infernoAmmo(weapon,strategy,pool,scanned):(normalizedTask.equals("blue dragon")||normalizedTask.equals("blue dragons"))&&style==CombatStyle.MAGIC?passiveAmmo(weapon,strategy,pool,scanned):buildAmmoItem(style,weapon,strategy,pool,scanned));
layout.add(toEquipmentItem(weapon,weaponRequirement==null?strategy!=null?strategy.getMethod():style==CombatStyle.MAGIC?"Magic weapon":style==CombatStyle.RANGED?"Ranged weapon":"Melee weapon":weaponRequirement.displayName,scanned));
layout.add(auditEquipment(task,EquipmentInventorySlot.BODY,null,style==CombatStyle.MAGIC?"Magic body":style==CombatStyle.RANGED?"Ranged body":"Melee body",strategy,style,equipped,pool,scanned,false,weapon));
...
}else{layout.add(auditEquipment(task,EquipmentInventorySlot.SHIELD,shieldRequirement,style==CombatStyle.MAGIC?"Magic off-hand":style==CombatStyle.RANGED?text(322):text(323),strategy,style,equipped,pool,scanned,false,weapon));
}layout.add(auditEquipment(task,EquipmentInventorySlot.LEGS,null,style==CombatStyle.MAGIC?"Magic legs":style==CombatStyle.RANGED?"Ranged legs":"Melee legs",strategy,style,equipped,pool,scanned,false,weapon));
...
layout.add(auditEquipment(task,EquipmentInventorySlot.BOOTS,requirementForSlot(requirements,EquipmentInventorySlot.BOOTS),style==CombatStyle.MAGIC?"Magic boots":style==CombatStyle.RANGED?"Ranged boots":"Melee boots",strategy,style,equipped,pool,scanned,false,weapon));
layout.add(auditEquipment(task,EquipmentInventorySlot.RING,null,style==CombatStyle.MAGIC?"Magic ring":style==CombatStyle.RANGED?"Ranged ring":"Melee ring",strategy,style,equipped,pool,scanned,false,weapon));
KitItem extraQuiverAmmo=buildDizanaExtraAmmoItem(false,style,weapon,strategy,pool,scanned);
...
selectedRank=rank;
}}if(selected!=null&&selectedRank<Integer.MAX_VALUE){return selected.toItem(1);
...
return toEquipmentItem(selected,placeholder,scanned);
...
for(OwnedItem item:pool){if((!seekingAlternative||item.isExactSeekingArrow())&&item.named(fragment)&&(item.equipmentSlot==EXTRA_QUIVER_AMMO_SLOT||item.matchesSlot(EquipmentInventorySlot.AMMO))){return item.toItem(1);
...
if(name.contains("broken")||name.contains("mangled")||name.contains("depleted")||name.contains("inactive")||name.contains("max hood")){return false;
...
}}return auditEquipment(task,EquipmentInventorySlot.CAPE,null,style==CombatStyle.RANGED?text(342):style==CombatStyle.MAGIC?"Magic cape":"Melee cape",strategy,style,Collections.emptyList(),pool,scanned,false,weapon);
...
break;
}if(strategy!=null&&slot!=EquipmentInventorySlot.WEAPON){int defence=stats.getDstab()+stats.getDslash()+stats.getDcrush()+stats.getDmagic()+stats.getDrange();
...
String name=item.nameKey;
if(slot==EquipmentInventorySlot.WEAPON){switch(style){case RANGED:return name.contains("bow")||name.contains("crossbow")||name.contains("blowpipe")||name.contains("ballista")||name.contains("atlatl")||name.contains(text(300))||(stats!=null&&(stats.getArange()>0||stats.getRstr()>0));
case MAGIC:return name.contains("staff")||name.contains("wand")||name.contains("trident")||name.contains("sceptre")||name.contains("sanguinesti")||name.contains("tumeken")||(stats!=null&&(stats.getAmagic()>0||stats.getMdmg()>0));
case MELEE:return name.contains("scythe")||name.contains("axe")||name.contains("sword")||name.contains("whip")||name.contains("rapier")||name.contains("mace")||name.contains("spear")||name.contains("hasta")||name.contains("halberd")||name.contains("fang")||name.contains("claws")||(stats!=null&&(Math.max(stats.getAstab(),Math.max(stats.getAslash(),stats.getAcrush()))>0||stats.getStr()>0));
case FLEXIBLE:default:return true;
}}switch(style){case RANGED:return name.contains("buckler")||name.contains("dragonfire ward")||name.contains("odium ward")||name.contains("book of law")||(stats!=null&&(stats.getArange()>0||stats.getRstr()>0));
case MAGIC:return name.contains("elidinis")||name.contains("mage s book")||name.contains("book of darkness")||name.contains(text(343))||name.contains(text(344))||name.contains(text(345))||(stats!=null&&(stats.getAmagic()>0||stats.getMdmg()>0));
case MELEE:return name.contains("defender")||name.contains("dragonfire shield")||name.contains(text(346))||name.contains("spectral spirit shield")||name.contains("crystal shield")||name.contains(text(347))||(stats!=null&&(Math.max(stats.getAstab(),Math.max(stats.getAslash(),stats.getAcrush()))>0||stats.getStr()>0));
case FLEXIBLE:default:return true;
...
int defence=stats.getDstab()+stats.getDslash()+stats.getDcrush()+stats.getDmagic()+stats.getDrange();
long speedBonus=slot==EquipmentInventorySlot.WEAPON&&stats.getAspeed()>0?Math.max(0,8-stats.getAspeed())*40L:0L;
return stats.getStr()*10_000L+attack*120L+stats.getPrayer()*40L+defence+speedBonus;
}private static long rangedScore(ItemEquipmentStats stats,EquipmentInventorySlot slot){int defence=stats.getDstab()+stats.getDslash()+stats.getDcrush()+stats.getDmagic()+stats.getDrange();
long speedBonus=slot==EquipmentInventorySlot.WEAPON&&stats.getAspeed()>0?Math.max(0,8-stats.getAspeed())*40L:0L;
return stats.getRstr()*10_000L+stats.getArange()*120L+stats.getPrayer()*40L+defence+speedBonus;
}private static long magicScore(ItemEquipmentStats stats,EquipmentInventorySlot slot){int defence=stats.getDstab()+stats.getDslash()+stats.getDcrush()+stats.getDmagic()+stats.getDrange();
long speedBonus=slot==EquipmentInventorySlot.WEAPON&&stats.getAspeed()>0?Math.max(0,8-stats.getAspeed())*40L:0L;
return Math.round(stats.getMdmg()*10_000.0)+stats.getAmagic()*120L+stats.getPrayer()*40L+defence+speedBonus;
...
}}for(Requirement requirement:requirements){if(!requirement.equipment){addItem(layout,choose(requirement.displayName,requirement.quantity,pool,scanned,requirement.alternatives));
}}boolean resolvedCannonMethod=cannonSuggested||methodRules.usesCannon();
if(resolvedCannonMethod){addItem(layout,choose("Cannon base",1,pool,scanned,"cannon base"));
addItem(layout,choose("Cannon stand",1,pool,scanned,"cannon stand"));
addItem(layout,choose("Cannon barrels",1,pool,scanned,"cannon barrels"));
addItem(layout,choose("Cannon furnace",1,pool,scanned,"cannon furnace"));
...
}RunePolicy.Resolution resolvedRunePackage=RunePolicy.resolve(methodRules.getPouchRunes(),ownedItemQuantities(pool));
...
continue;
}addItem(layout,choose(rune.getName()+" runes",rune.getMinimumQuantity(),pool,scanned,normalize(rune.getName())+" rune"));
...
}KitItem meal=choose(methodRules.getFoodDisplayName(),1,pool,scanned,methodRules.getFoodAlternatives());
...
}private static List<KitItem>enforceConcreteInventoryTarget(List<KitItem>source,MethodRules rules,KitItem prayerRestore,KitItem food){if(source==null||source.isEmpty()||rules==null){return source;
...
}if(item.hasItemId()){concrete.add(item);
...
}}}}if(name.contains("rune pouch")||name.contains(text(10))||name.contains("herb sack")||name.contains("gem bag")||name.contains("bonecrusher")||name.contains("goading potion")){return MethodRules.InventoryGroup.UTILITY;
}if(name.contains("sceptre")||name.contains("switch")){return MethodRules.InventoryGroup.SWITCH;
}if(name.contains(" rune")||name.endsWith("runes")||name.contains("cannon")||name.contains("dart")||name.contains("arrow")||name.contains("bolt")){return MethodRules.InventoryGroup.RUNES;
}if(name.contains(text(17))){return MethodRules.InventoryGroup.FOOD;
}if(name.contains("heart")||name.contains("combat potion")||name.contains("ranging potion")||name.contains("magic potion")||name.contains("bastion potion")||name.contains(text(18))||name.contains("ancient brew")||name.contains("forgotten brew")){return MethodRules.InventoryGroup.BOOST;
}if(name.contains("antivenom")||name.contains("anti venom")||name.contains("antipoison")||name.contains("stamina")||name.contains("antifire")){return MethodRules.InventoryGroup.PROTECTION;
}if(name.contains("super restore")||name.contains("prayer potion")||name.contains("prayer restoration")||name.contains("prayer regeneration")){return MethodRules.InventoryGroup.RESTORE;
...
}if(description.contains("blowpipe")||description.contains("shortbow")||description.contains("longbow")||description.contains("crossbow")||description.contains("ballista")||description.contains(" ranged ")||description.startsWith("ranged ")||description.contains(" range switch")||description.contains("masori")||description.contains("armadyl")||description.contains("karil")){return KitItem.SwitchStyle.RANGED;
}if(description.contains(" magic ")||description.startsWith("magic ")||description.contains(" mage ")||description.startsWith("mage ")||description.contains("magicks")||description.contains("ancient")||description.contains("sceptre")||description.contains("staff")||description.contains("wand")||description.contains("ancestral")||description.contains("virtus")||description.contains("ahrim")||description.contains("bloodbark")){return KitItem.SwitchStyle.MAGIC;
}if(description.contains(" melee ")||description.startsWith("melee ")||description.contains("scythe")||description.contains("godsword")||description.contains("whip")||description.contains("claws")||description.contains("defender")||description.contains("torva")||description.contains("bandos")){return KitItem.SwitchStyle.MELEE;
...
}private static boolean isFoodName(String name){return name.contains("anglerfish")||name.contains("manta ray")||name.contains("dark crab")||name.contains("shark")||name.contains("sea turtle")||name.contains("karambwan")||name.contains("monkfish")||name.contains("high healing food");
}private List<KitItem>buildOptionalLayout(String task,TaskStrategy strategy,List<OwnedItem>pool,OwnedItem weapon){List<KitItem>optional=new ArrayList<>();
...
}private static boolean isAshSanctifierTask(String assignment){String task=normalize(assignment);
return task.contains("demon")||task.contains("hellhound")||task.contains("bloodveld")||task.contains(text(364))||task.contains("smoke devil")||task.contains(text(365))||task.contains("fiend")||task.contains("cerberus");
}private static boolean isBonecrusherTask(String assignment){String task=normalize(assignment);
return task.contains("dragon")||task.contains("wyvern")||task.contains("wyrm")||task.contains("drake")||task.contains("hydra")||task.contains("dagannoth")||task.contains("kalphite")||task.contains("basilisk")||task.contains("giant")||task.contains("troll")||task.contains("bat")||task.contains("mole")||task.contains("ankou");
...
if(normalizedTask.equals(text(263))){return choose(text(366),1,pool,scanned,text(367),"dramen staff","lunar staff");
}if(normalizedTask.equals("tormented demons")){OwnedItem direct=exactOwned(pool,text(350));
return direct==null?choose(text(368),1,pool,scanned,"games necklace"):direct.toItem(1);
}List<TravelRoutes.Option>reviewedOptions=TravelRoutes.fallbacksFor(location);
if(!reviewedOptions.isEmpty()){List<String>alternatives=new ArrayList<>();
for(TravelRoutes.Option option:reviewedOptions){if(diaries.allowsTravelItem(option.getItemFamily())){alternatives.add(option.getItemFamily());
}}if(alternatives.isEmpty()){return null;
}TravelRoutes.Option preferred=reviewedOptions.get(0);
for(TravelRoutes.Option option:reviewedOptions){if(diaries.allowsTravelItem(option.getItemFamily())){preferred=option;
break;
}}return choose(preferred.getItemFamily()+" -> "+preferred.getDestination(),1,pool,scanned,alternatives);
}if(combined.contains("cowbell")){return choose(text(369),1,pool,scanned,text(370));
}if(combined.contains("giantsoul")){return choose(text(366),1,pool,scanned,text(367),"dramen staff","lunar staff");
}if(combined.contains(text(371))){return choose(text(372),1,pool,scanned,text(350),"games necklace");
}if(combined.contains("key master")){return choose("Key master teleport",1,pool,scanned,"key master teleport","games necklace");
}if(combined.contains("barrows teleport")){return choose("Barrows teleport",1,pool,scanned,"barrows teleport",text(373),text(374));
}if(combined.contains("ring of shadows")||combined.contains("ancient vault")){return choose("Ring of shadows",1,pool,scanned,"ring of shadows");
}if(combined.contains(text(375))||combined.contains(text(376))){return choose("Wilderness travel",1,pool,scanned,text(375),text(377),"seed pod");
}if(combined.contains("digsite pendant")){return choose("Digsite pendant",1,pool,scanned,"digsite pendant");
}if(combined.contains("drakan")){return choose(text(378),1,pool,scanned,text(379));
}if(combined.contains("varrock teleport")){return choose("Varrock teleport",1,pool,scanned,"varrock teleport","varrock tablet");
}if(combined.contains(text(30))){return choose(text(380),1,pool,scanned,text(30));
}if(combined.contains(text(381))){return choose(text(382),1,pool,scanned,text(381));
}if(combined.contains("skills necklace")){return choose("Skills necklace",1,pool,scanned,"skills necklace");
}if(combined.contains("combat bracelet")){return choose("Combat bracelet",1,pool,scanned,"combat bracelet");
}if(combined.contains("ectophial")){return choose("Ectophial",1,pool,scanned,"ectophial");
}if(combined.contains("mort ton teleport")){return choose(text(383),1,pool,scanned,"mort ton teleport");
}if(combined.contains(text(384))){return choose(text(385),1,pool,scanned,text(384));
}if(combined.contains("zul andra")){return choose(text(386),1,pool,scanned,text(387));
}if(combined.contains("catacomb")||combined.contains("xeric")){return choose(text(388),1,pool,scanned,text(389));
}if(combined.contains(text(390))||combined.contains("rada")){return choose(text(391),1,pool,scanned,text(35));
}if(combined.contains("fremennik")||combined.contains("slayer cave")){return choose("Slayer ring",1,pool,scanned,"slayer ring");
}if(combined.contains("fairy ring")){if(diaries.hasTier("lumbridge",4)){return null;
}return choose(text(392),1,pool,scanned,"lunar staff","dramen staff");
}return choose("Teleport out",1,pool,scanned,"max cape","construction cape",text(393),"house tab","slayer ring",text(381),"games necklace");
...
}private static boolean isPotionFamilyName(String name){return name.contains("potion")||name.contains("super restore")||name.contains("brew")||name.contains("serum")||name.contains("antidote")||name.contains("antivenom")||name.contains("anti venom")||name.contains("antipoison");
...
if(slot==EquipmentInventorySlot.HEAD&&(name.contains("earmuff")||name.contains("face mask")||name.contains("nose peg")||name.contains("spiny helmet")||name.contains("goggle"))){return requirement;
...
return text.contains("magic")||text.contains("mage")||text.contains("powered staff")||text.contains("tumeken")||text.contains("trident");
...
if(base&&stand&&barrels&&furnace&&cannonballs){return text(410);
}if(!scanned){return text(411);
}List<String>missing=new ArrayList<>();
if(!(base&&stand&&barrels&&furnace)){missing.add("pieces");
...
}private List<Requirement>requirementsFor(String task,String location,CombatStyle style,int remainingKills,boolean desertElite){List<Requirement>requirements=new ArrayList<>();
...
}}if(task.contains("lizard")&&!task.contains("lizardmen")){requirements.add(inventoryNeed(consumableFinisherQuantity(remainingKills),"Ice cooler","ice cooler"));
...
}if(task.contains("kurask")||task.contains("turoth")){if(style==CombatStyle.RANGED){requirements.add(gearRequirement("Broad ammunition","broad bolts",text(311),"broad arrows",text(423)));
...
}private static boolean usesReviewedDragonPackage(String task){return task.equals("blue dragon")||task.equals("blue dragons")||task.equals("black dragon")||task.equals("black dragons")||task.equals("green dragon")||task.equals("green dragons")||task.equals("red dragon")||task.equals("red dragons")||task.equals("metal dragon")||task.equals("metal dragons")||task.equals("bronze dragon")||task.equals("bronze dragons")||task.equals("iron dragon")||task.equals("iron dragons")||task.equals("steel dragon")||task.equals("steel dragons")||task.equals("mithril dragon")||task.equals("mithril dragons")||task.equals("adamant dragon")||task.equals("adamant dragons")||task.equals("rune dragon")||task.equals("rune dragons");
...
}private enum CombatStyle{MAGIC("magic"),RANGED("ranged"),MELEE("melee"),FLEXIBLE("flexible");
...
this.weapon=weapon;
this.score=score;
}}private static final class OwnedItem{private final int itemId;
...
private OwnedItem(int itemId,String displayName,int quantity,KitItem.Status status,int equipmentSlot,ItemEquipmentStats equipmentStats,Set<String>normalizedNames){this.itemId=itemId;
...
}private boolean matches(String task,String location,CombatStyle style){String value=scope.equals("location")?location:task;
return value.contains(match)&&(style.equals("*")||style.equals(style.name().toLowerCase(Locale.ROOT)));
}private Requirement create(int remainingKills){int amount=quantity.equals("F")?consumableFinisherQuantity(remainingKills):Integer.parseInt(quantity);
...
}private static final class Requirement{private final String displayName;
private final boolean equipment;
private final int quantity;
...
}static List<String>araxxorSafeAmmunitionForTest(String safeWeaponName){return new ArrayList<>(araxxorSafeAmmunitionAlternatives(safeWeaponName));
