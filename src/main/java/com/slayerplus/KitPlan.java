...
}KitPlan withRemainingFinishers(int remaining){List<KitItem>updated=null;
for(int i=0;i<inventoryItems.size();i++){KitItem item=inventoryItems.get(i);
String name=item.getDisplayName().toLowerCase(Locale.ROOT);
int quantity=name.equals("fungicide")?1+(Math.max(1,remaining)-1)/10:name.equals("bag of salt")||name.equals("ice cooler")||name.contains("fishing explosive")?Math.max(1,remaining):item.getQuantity();
quantity=Math.min(item.getQuantity(),quantity);
if(quantity!=item.getQuantity()){if(updated==null){updated=new ArrayList<>(inventoryItems);
}updated.set(i,item.withQuantity(quantity));
}}return updated==null?this:new KitPlan(equipment,inventory,ownedStatus,layoutTitle,equipmentItems,updated,optionalItems);
