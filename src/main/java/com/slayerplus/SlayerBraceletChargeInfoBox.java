...
final class SlayerBraceletChargeInfoBox extends InfoBox
{
	...
	void setCharges(final int charges)
	{
		...
		setTooltip(braceletName + ": "
			+ (charges < 0 ? "right-click Check to learn charges"
	...
	public Color getTextColor()
	{
		if (charges == 0)
		{
			return depletedTextColorForRegression(System.currentTimeMillis());
		}
	...
	static Color depletedTextColorForRegression(final long nowMillis)
	{
		final double phase = (nowMillis % DEPLETED_PULSE_PERIOD_MILLIS)
			/ (double) DEPLETED_PULSE_PERIOD_MILLIS;
		final double blend = (Math.sin(phase * Math.PI * 2.0) + 1.0) / 2.0;
	...
	private static BufferedImage dullDepletedImage(final BufferedImage source)
	{
		if (source == null)
		{
			return null;
		...
		for (int y = 0; y < source.getHeight(); y++)
		{
			for (int x = 0; x < source.getWidth(); x++)
			{
				...
				final int grey = (red * 30 + green * 59 + blue * 11) / 100;
				final int mutedRed = Math.min(255, grey * 3 / 4 + 45);
				final int mutedOther = grey * 3 / 5;
				result.setRGB(x, y, alpha << 24 | mutedRed << 16
					| mutedOther << 8 | mutedOther);
			}
		}
		return result;
	}
