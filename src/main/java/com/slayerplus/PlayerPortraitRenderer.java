...
public final class PlayerPortraitRenderer {
  ...
  public BufferedImage render(Model inputModel) {
    return render(inputModel, inputModel, true);
  }

  public BufferedImage render(Model inputModel, boolean allowCalibration) {
    return render(inputModel, inputModel, allowCalibration);
  }
  ...
  public BufferedImage render(
    ...
    try {
      ...
      if (lockedContentBounds == null) {
        BufferedImage calibrationCapture =
            renderCaptureSized(
                bodyModel,
                rasterizer,
                pixels,
                ...
                CAPTURE_SIZE,
                lockedFraming);
        BufferedImage calibrationPortrait =
      ...
      BufferedImage fullCapture =
          renderCaptureSized(
              ...
              extendedCaptureSize,
              supersampledFraming);
      BufferedImage supersampledPortrait =
      ...
      BufferedImage fallbackBodyFront =
          renderCaptureSized(
              bodyModel,
              rasterizer,
              pixels,
              ...
              lockedFraming);
      neckTurn = applyTemporaryNeckTurn(bodyModel);
      BufferedImage fallbackTurnedCapture =
          renderCaptureSized(
              bodyModel,
              rasterizer,
              pixels,
  ...
  private static BufferedImage renderSupersampledComposite(
    ...
    BufferedImage refinedPortrait = resamplePremultiplied(intermediate, drawWidth, drawHeight);
    BufferedImage clarifiedPortrait = applyPortraitFinish(refinedPortrait);
    BufferedImage output =
        new BufferedImage(OUTPUT_SIZE, OUTPUT_SIZE, BufferedImage.TYPE_INT_ARGB_PRE);
  ...
  private static BufferedImage applyPortraitFinish(BufferedImage source) {
    BufferedImage clarified = applyInteriorClarity(source);
  ...
  private BufferedImage copyLockedCrop(
    ...
    BufferedImage scaledPortrait =
        new BufferedImage(drawWidth, extendedDrawHeight, BufferedImage.TYPE_INT_ARGB);
    Graphics2D scaledGraphics = scaledPortrait.createGraphics();
    ...
    BufferedImage refinedPortrait = applySubtleSharpen(scaledPortrait);
    BufferedImage output = new BufferedImage(OUTPUT_SIZE, OUTPUT_SIZE, BufferedImage.TYPE_INT_ARGB);
    Graphics2D graphics = output.createGraphics();
    graphics.setRenderingHint(
        RenderingHints.KEY_ALPHA_INTERPOLATION, RenderingHints.VALUE_ALPHA_INTERPOLATION_QUALITY);
