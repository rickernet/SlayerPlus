...
final class SlayerOptionalPins {
    ...
    static boolean allowsMenu(String activeTag, int componentId, int itemId, boolean shiftHeld) {
        return componentId == InterfaceID.Bankmain.ITEMS && itemId > 0
            && (shiftHeld || activeTag != null
    ...
    static KitPlan prepend(KitPlan plan, List<KitItem> pins) {
        ...
        return new KitPlan(plan.getEquipment(), plan.getInventory(), plan.getOwnedStatus(),
            plan.getLayoutTitle(), plan.getEquipmentItems(), plan.getInventoryItems(),
