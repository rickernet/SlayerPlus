...
panel=new SlayerPlusPanel(()->clientThread.invokeLater(this::createRecommendedBankTag),variant->clientThread.invokeLater(()->selectBankTagVariant(variant)),config,configs);
...
layoutService=null;
resetSessionState();
}private void resetSessionState(){layoutCreated=false;
...
}int getTravelItemIdForOverlay(){KitPlan plan=currentLoadout;
...
@Subscribe public void onMenuOpened(MenuOpened event){if(event==null){return;
}int travelItemId=getTravelItemIdForOverlay();
if(travelItemId<=0){return;
}String destination=travelDestination();
MenuEntry[]entries=event.getMenuEntries();
if(entries==null){return;
}for(MenuEntry entry:entries){highlightTravelDestination(entry,travelItemId,destination);
}}private boolean highlightTravelDestination(MenuEntry entry,int travelItemId,String destination){if(entry==null){return false;
}boolean childMatched=false;
Menu subMenu=entry.getSubMenu();
MenuEntry[]children=subMenu==null?null:subMenu.getMenuEntries();
if(children!=null){for(MenuEntry child:children){if(highlightTravelDestination(child,travelItemId,destination)){childMatched=true;
}}}boolean ownedByTravelItem=entry.getItemId()==travelItemId;
boolean leafMatch=ownedByTravelItem&&matchesTravelDestination(entry,destination);
boolean unambiguousAction=ownedByTravelItem&&(children==null||children.length==0)&&isTravelActionVerb(entry.getOption());
