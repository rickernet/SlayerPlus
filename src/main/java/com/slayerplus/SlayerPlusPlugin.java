...
return;
}if(needsBankRestock()){Player player=client.getLocalPlayer();
WorldPoint nearestBank=player==null?null:BankRoutes.nearest(player.getWorldLocation());
...
WorldPoint waypoint=target==null?null:SlayerTaskWaypoints.find(target.assignment,target.location);
if(waypoint==null){shortestPathBridge.clear();
return;
}shortestPathBridge.routeTo(waypoint,true);
}private boolean needsBankRestock(){KitPlan plan=currentLoadout;
return plan!=null&&plan.getUnresolvedItemCount()>0;
