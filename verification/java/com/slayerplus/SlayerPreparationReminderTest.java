...
public class SlayerPreparationReminderTest {
    @Test public void runeConsumptionStopsSchedulingRefreshesAfterSetup() {
    ...
    @Test public void fulfilledTaskDoesNotRecheckDuringCombatOrWorldHop() {
        for (int remaining = 100; remaining > 0; remaining--) {
            assertFalse(SlayerPlusPlugin.shouldRefreshPreparation(true, "task", "task", remaining + 1, remaining));
        }
    ...
    @Test public void pendingAndNewTaskRequirementsAreCheckedAgain() {
    ...
    @Test public void cachedRowsSurviveRenderingAndEquivalentUpdates() {
    ...
    @Test public void panelHidesWhenReadyAndReturnsForAnotherIncompleteSetup() {
        SlayerTaskReadinessOverlay overlay = new SlayerTaskReadinessOverlay(new SlayerPlusPlugin());
    ...
    private static PreparationCatalog.PreparationPlan plan(boolean ready, boolean bookReady, boolean runesReady) {
        return new PreparationCatalog.PreparationPlan(true, ready, "Setup", ready ? "Ready" : "Check spellbook and rune pouch.",
            "Load Water, Blood and Death runes", "", Collections.emptyList(), MethodRules.Spellbook.ANCIENT,
            bookReady, true, 99, Arrays.asList(
                new PreparationCatalog.RuneStatus("Water", 6, runesReady ? 600 : 0),
                new PreparationCatalog.RuneStatus("Blood", 2, runesReady ? 200 : 0),
                new PreparationCatalog.RuneStatus("Death", 4, runesReady ? 400 : 0)), "Ice Barrage", runesReady ? 100 : 0, runesReady);
    }
}
