...
public class PluginServiceWiringTest {
  ...
  public void readsExistingSlayerServiceWithoutReinjectingPlugin() {
    ...
    slayer.injectorForTest(
        Guice.createInjector(
            binder -> binder.bind(SlayerPluginService.class).toInstance(slayer.service)));
    assertSame(
        slayer.service,
        SlayerPlusPlugin.findExistingSlayerService(Arrays.asList(new Plugin() {}, slayer)));
  ...
  public void absentSlayerServiceAllowsExistingTaskFallbacks() {
    ...
    assertNull(
        SlayerPlusPlugin.findExistingSlayerService(
  ...
  public void singleDependencyChildrenShareTheSelectedBankTab() {
    Injector bankScope =
        Guice.createInjector(binder -> binder.bind(TabState.class).in(Singleton.class));
  ...
  private static class ExistingSlayerPlugin extends SlayerPlugin {
    ...
    void injectorForTest(Injector value) {
      injector = value;
    }

    final SlayerPluginService service =
        new SlayerPluginService() {
          public List<NPC> getTargets() {
            return Collections.emptyList();
          }

          public String getTask() {
            return "Cave crawlers";
          }
