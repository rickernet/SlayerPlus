...
public class AraxxorLoadoutTest {
  @Test
  public void regularAraxytesTargetShortcutStartWithoutChangingAraxxor() {
    for (int agility : new int[] {62, 63, 99}) {
      List<WorldPoint> path =
          SlayerTaskWaypoints.findPath("Araxytes", "Morytania Spider Cave", agility, 126);
      assertEquals(new WorldPoint(3678, 9820, 0), path.get(path.size() - 1));
    }
    assertEquals(
        new WorldPoint(3630, 9813, 0),
        SlayerTaskWaypoints.findSpecific("Araxxor", "Morytania Spider Cave"));
  }
  ...
  @Test
  public void closingBankPreventsRestockRouteFromOverridingTaskTeleport() {
    SlayerPlusPlugin plugin = new SlayerPlusPlugin();
  ...
  @Test
  public void ownedScrollBeatsMaxCapeForBothTaskTypes() {
  ...
  @Test
  public void missingScrollRetainsNormalFallback() {
  ...
  @Test
  public void meleeAraxyteSwitchDoesNotNeedRangingPotion() {
  ...
  @Test
  public void rangedAraxyteSwitchKeepsRangingPotion() {
  ...
  @Test
  public void braceletsStayOptionalAndFoodNeverBecomesEquipment() {
    ...
    assertEquals(
        1,
        count(
            analyzer.buildOptionalLayout("Araxxor", strategy("Araxxor"), pool, null),
            ItemID.BRACELET_OF_SLAUGHTER));
    List<KitItem> gear =
        SlayerLoadoutAnalyzer.buildEquipmentLayout(
            "Araxxor",
            strategy("Araxxor"),
            SlayerLoadoutAnalyzer.CombatStyle.MELEE,
    ...
    List<BankTagLayout.InventoryPlacement> slots =
  ...
  @Test
  public void hallowedCrystalIsNotAUsableTravelRecommendation() {
  ...
  private List<SlayerLoadoutAnalyzer.OwnedItem> pool() {
    return Arrays.asList(
        owned(ItemID.TELEPORTSCROLL_SPIDERCAVE, "Spider cave teleport", null),
        ...
        owned(ItemID.TBWT_COOKED_KARAMBWAN, "Cooked karambwan", null),
