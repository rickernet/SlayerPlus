...
public class SlayerOptionalPinsTest {
    @Test public void menuNeedsSlayerTagOrShiftAndAlwaysNeedsBankItem() {
        ...
        assertFalse(SlayerOptionalPins.allowsMenu(BankTagLayout.TAG_NAME, InterfaceID.Bankmain.ITEMS_CONTAINER, ItemID.SHARK, false));
        assertFalse(SlayerOptionalPins.allowsMenu(null, InterfaceID.Bankmain.ITEMS_CONTAINER, ItemID.SHARK, true));
        assertFalse(SlayerOptionalPins.allowsMenu(BankTagLayout.TAG_NAME, InterfaceID.Bankmain.TABS, ItemID.SHARK, true));
    }

    @Test public void triggerAddsOneEntryAndDoesNotReenterForItsOwnOption() {
        ...
        for (String option : Arrays.asList("Withdraw-1", "Withdraw-All", "Cancel", "Pin to SlayerPlus optional row", "Unpin from SlayerPlus optional row", null)) {
            assertFalse(SlayerOptionalPins.isMenuTrigger(option, false));
        }
    }

    @Test public void pinsRoundTripInOrderAndRejectInvalidIds() {
        Set<Integer> pins = SlayerOptionalPins.read(ItemID.SHARK + ",bad,-1,0," + ItemID.SHARK + "," + ItemID.BH_RUNE_POUCH);
    ...
    @Test public void pinsLeadOptionalRowWithoutChangingRequiredLoadout() {
        KitItem shark = item(ItemID.SHARK);
        KitItem pouch = item(ItemID.BH_RUNE_POUCH);
        KitPlan original = new KitPlan("gear", "inventory", "owned", "task",
            Collections.singletonList(shark), Collections.singletonList(pouch),
    ...
    @Test public void pinnedEquipmentKeepsBothOptionalAndEquipmentPositions() {
        ...
        assertFalse(BankTagLayout.placeEquipmentItemId(layout, tags, ItemID.SHARK, 18, Collections.emptySet()));
