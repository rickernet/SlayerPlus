...
public class BarrowsInventoryTest {
    ...
    private final TaskStrategy strategy = SlayerTaskStrategyCatalog.resolve("Barrows Brothers", Preference.Playstyle.FAST_XP,
        Preference.Cannon.NEVER, Preference.Burst.NEVER, Preference.CombatStyle.AUTOMATIC, "Barrows", false);

    @Test public void shadowGetsOneCapeMagicBoostAndLootPouchWithoutRangedSwitch() {
        ...
        assertFalse(items.stream().anyMatch(item -> item.isEquipmentSwitch() && item.getSwitchStyle() == KitItem.SwitchStyle.RANGED));
        assertFalse(PreparationCatalog.resolve("Barrows Brothers", "Barrows", strategy,
            null, null, null, Collections.emptyMap(), ItemID.TUMEKENS_SHADOW).isActive());
    }

    @Test public void airSpellsSkipRangedAndPoweredStaffBoostButRetainCastingRequirements() {
        List<KitItem> items = inventory(ItemID.SMOKE_BATTLESTAFF, base());
        assertEquals(0, count(items, ItemID._4DOSERANGERSPOTION));
        assertEquals(0, count(items, ItemID.SATURATED_HEART));
        assertFalse(items.stream().anyMatch(item -> item.isEquipmentSwitch() && item.getSwitchStyle() == KitItem.SwitchStyle.RANGED));
        assertTrue(PreparationCatalog.resolve("Barrows Brothers", "Barrows", strategy,
            null, null, null, Collections.emptyMap(), ItemID.SMOKE_BATTLESTAFF).isActive());
    }

    @Test public void otherPoweredStavesGetCompatibleRangedSwitchAmmunition() {
        Object[][] cases = {
            ...
            {ItemID.DTTD_BONE_CROSSBOW, "Dorgeshuun crossbow", ItemID.DTTD_BONE_CROSSBOW_BOLT, "Bone bolts"},
            {ItemID.HUNTING_CROSSBOW_SUNLIGHT, "Hunters' sunlight crossbow", ItemID.SUNLIGHT_ANTELOPE_BOLT, "Sunlight antler bolts"},
            {ItemID.XBOWS_CROSSBOW_RUNITE, "Rune crossbow", ItemID.XBOWS_CROSSBOW_BOLTS_RUNITE, "Runite bolts"}
        };
    ...
    @Test public void blowpipeNeedsNoLooseAmmoAndRangedArmourOnlyAppearsForRangedSwitch() {
        List<SlayerLoadoutAnalyzer.OwnedItem> pool = base();
    ...
    @Test public void optionalLockpickIsOnlyAddedWhenOwnedAndSpadeStaysRequired() {
        List<SlayerLoadoutAnalyzer.OwnedItem> pool = base();
        List<KitItem> items = inventory(ItemID.TUMEKENS_SHADOW, pool);
        ...
        assertTrue(inventory(ItemID.TUMEKENS_SHADOW, Collections.emptyList()).stream()
            .anyMatch(item -> item.getDisplayName().equals("Spade") && !item.hasItemId()));
    }

    @Test public void prayerPotionFallbackRetainsStatRestoreAndMissingLootPouchIsNotRequired() {
        List<SlayerLoadoutAnalyzer.OwnedItem> pool = Arrays.asList(
            owned(ItemID._4DOSEPRAYERRESTORE, "Prayer potion(4)", null),
            owned(ItemID._4DOSESTATRESTORE, "Restore potion(4)", null), owned(ItemID.SHARK, "Shark", null));
        List<KitItem> items = inventory(ItemID.TUMEKENS_SHADOW, pool);
    ...
    private List<KitItem> inventory(int weapon, List<SlayerLoadoutAnalyzer.OwnedItem> pool) {
        List<KitItem> gear = Arrays.asList(null, null, null, null, new KitItem("Weapon", weapon, 1, KitItem.Status.BANK));
        List<KitItem> items = analyzer.buildInventoryLayout("Barrows Brothers", strategy, "Barrows", "", SlayerLoadoutAnalyzer.CombatStyle.MAGIC,
            Collections.emptyList(), false, 8, pool, true, gear);
        assertTrue(items.size() <= 28);
        return items;
    }

    private static List<SlayerLoadoutAnalyzer.OwnedItem> base() {
        return new ArrayList<>(Arrays.asList(owned(ItemID.SKILLCAPE_MAX, "Max cape", EquipmentInventorySlot.CAPE),
            owned(ItemID.SATURATED_HEART, "Saturated heart", null), owned(ItemID.BH_RUNE_POUCH, "Rune pouch", null),
            owned(ItemID.SPADE, "Spade", null), owned(ItemID.SHARK, "Shark", null),
            owned(ItemID._4DOSE2RESTORE, "Super restore(4)", null), owned(ItemID._4DOSESTATRESTORE, "Restore potion(4)", null),
