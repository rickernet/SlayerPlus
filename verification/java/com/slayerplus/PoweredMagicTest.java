...
public class PoweredMagicTest {
    @Test public void actualBarrowsCatalogFollowsRecommendedWeapon() {
        TaskStrategy strategy = SlayerTaskStrategyCatalog.resolve("Barrows Brothers", Preference.Playstyle.FAST_XP,
        ...
        assertTrue(PreparationCatalog.resolve("Barrows Brothers", "Barrows", strategy,
            null, null, null, Collections.emptyMap(), ItemID.SLAYER_STAFF).isActive());
    }

    @Test public void sharedRuleUsesSelectedWeaponForEveryTask() {
        TaskStrategy strategy = strategy();
        for (String task : Arrays.asList("Barrows Brothers", "Fire giants", "Blue dragons")) {
            for (int weapon : new int[] {ItemID.TUMEKENS_SHADOW, ItemID.TOTS_CHARGED,
                ItemID.SANGUINESTI_STAFF, ItemID.WARPED_SCEPTRE, ItemID.EYE_OF_AYAK,
                ItemID.WILD_CAVE_SCEPTRE_CHARGED, ItemID.WILD_CAVE_ACCURSED_CHARGED, ItemID.RAT_BONE_STAFF}) {
                ...
                assertFalse(task, PreparationCatalog.resolve(task, "", strategy,
                    null, null, null, Collections.emptyMap(), weapon).isActive());
            }
    ...
    @Test public void explicitCombatPackagesAreRemovedButUtilityAndFreezesStay() {
        for (String spell : Arrays.asList("Fire Surge", "Water Wave / Fire Wave", "Magic Dart")) {
            MethodRules rules = spell(MethodRules.Spellbook.STANDARD, spell).withoutStandardCombatSpell().build();
        ...
        for (String utility : Arrays.asList("Crumble Undead", "High Level Alchemy")) {
            MethodRules rules = spell(MethodRules.Spellbook.STANDARD, utility).withoutStandardCombatSpell().build();
            assertEquals(MethodRules.Spellbook.STANDARD, rules.getSpellbook());
            assertFalse(rules.getPouchRunes().isEmpty());
        }
        for (MethodRules.Spellbook book : Arrays.asList(MethodRules.Spellbook.ANCIENT, MethodRules.Spellbook.ARCEUUS)) {
        ...
        MethodRules thralls = SlayerMethodRuleCatalog.resolve("Zulrah", "", strategy(), ItemID.TUMEKENS_SHADOW);
        assertEquals(MethodRules.Spellbook.ARCEUUS, thralls.getSpellbook());
        assertTrue(thralls.usesThralls());
        MethodRules alchemy = SlayerMethodRuleCatalog.resolve("Bloodveld", "", strategy(), ItemID.TUMEKENS_SHADOW);
        assertEquals(MethodRules.Spellbook.STANDARD, alchemy.getSpellbook());
        assertTrue(alchemy.getPouchRunes().stream().anyMatch(rune -> rune.getItemId() == ItemID.NATURERUNE));
        assertFalse(alchemy.getPouchRunes().stream().anyMatch(rune -> rune.getItemId() == ItemID.WRATHRUNE));
    }

    @Test public void inventoryAndReminderAgreeAndWeaponSlotIsNotAmmoSlot() {
        SlayerLoadoutAnalyzer analyzer = new SlayerLoadoutAnalyzer(null);
        List<KitItem> equipment = Arrays.asList(null, null, null,
    ...
    private static TaskStrategy strategy() {
        return TaskStrategy.builder(TaskStrategy.CombatStyle.MAGIC, "Use Air spells")
            .runePouch(true).reviewed("2026-09-02").build();
    }

    private static MethodRules.Builder spell(MethodRules.Spellbook book, String name) {
        return MethodRules.builder().spell(book, name).runePouch(true).pouchRune(ItemID.AIRRUNE, "Air", 100);
    }

    private static List<KitItem> inventory(SlayerLoadoutAnalyzer analyzer, List<KitItem> equipment) {
        return analyzer.buildInventoryLayout("Barrows Brothers", strategy(), "", "", SlayerLoadoutAnalyzer.CombatStyle.MAGIC,
            Collections.emptyList(), false, 8, Collections.emptyList(), true, equipment);
    }
